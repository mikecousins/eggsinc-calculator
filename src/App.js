          ...
          <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <div class="bg-teal-lightest border-t-4 border-teal rounded-b text-teal-darkest px-4 py-3 shadow-md" role="alert">
              <div class="flex">
                ...
                <div>
                  <p class="font-bold">Ending Egg Count - {new Intl.NumberFormat().format(endingEggCount)}</p>
                  <p class="text-sm">Ending Chicken Count - {new Intl.NumberFormat().format(endingPopulation)}</p>
                </div>
              </div>
            </div>
            <div className="mb-4 mt-4">
              <label className="block text-grey-darker text-sm font-bold mb-2">Current Chickens</label>
              <input className="shadow appearance-none border rounded w-full py-2 px-3 text-grey-darker leading-tight" type="text" value={this.state.currentChickens} onChange={e => this.setState({ currentChickens: e.target.value })} />
            </div>
            <div className="mb-4">
